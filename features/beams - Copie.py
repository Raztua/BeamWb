...
class BeamFeature:
    def __init__(self, obj):
        ...
        obj.addProperty("App::PropertyFloatList", "DiagramValues", "Diagram",
                        "List of values for diagram points", 4)
        ...
        obj.addProperty("App::PropertyFloat", "DiagramMax", "Diagram",
                        "Width of diagram ", 4).DiagramMax = 1
        self.flagInit = False
        self.execute(obj)
    ...
    def set_diagram(self, positions, values, max_value=None):
        """Set the diagram with actual values and scaling"""
        obj = self.Object
        # Store the actual values and positions
        obj.DiagramPositions = positions
        obj.DiagramValues = values

        # Store the max value for scaling
        if max_value is not None:
            obj.DiagramMax = max_value
...
class ResultBeam(BeamFeature):
    def __init__(self, obj, base_beam=None):
        ...
        obj.addProperty("App::PropertyFloat", "DiagramMaxValue", "Diagram",
                        "Maximum absolute value in diagram", 4)
        obj.addProperty("App::PropertyFloat", "DiagramMinValue", "Diagram",
                        "Minimum value in diagram", 4)
    ...
    def initialize_from_base_beam(self, obj, base_beam):
        ...
        obj.DiagramMaxValue = 0.0
        obj.DiagramMinValue = 0.0
        obj.MaxDiagramValue = 0.0
        obj.ResultType = "None"
...
class BeamViewProvider():
    ...
    def _add_result_text(self, obj, parent_sep, start_pos, end_pos, beam_dir):
        ...
        text_pos += App.Vector(0, 0, obj.DiagramMax * obj.DiagramWidth * 1.2)  # Offset above beam
    ...
    def createSectionModes(self, obj, section, start_pos, end_pos, rot):
        ...
        points, faces, _ = get_section_points(section.ProfileType, beam_length, params)
        # Create coordinates
        coords = coin.SoCoordinate3()
    ...
    def createDiagram(self, obj):
        """Create the diagram visualization with max value annotation"""
        from pivy import coin
        ...
        # FIX: Use DiagramMax for scaling instead of calculating max_value
        max_value = obj.DiagramMax if hasattr(obj, 'DiagramMax') and obj.DiagramMax != 0 else 1.0

        # Create main separator
        diagram_sep = coin.SoSeparator()
        diagram_sep.setName("Beam diagram separator")
        ...
        max_point = start_pos + beam_dir * max_pos
        max_offset = local_z * (1 if max_val >= 0 else -1) * (abs(max_val) / max_value) * diagram_width * beam_length
        max_value_point = max_point + max_offset
        ...
        # Start with zero point at beginning
        all_points.append(start_pos)
        line_points.append(start_pos)

        # Process each diagram point - FIX: Use normalized values
        ...
        for i, (pos, val) in enumerate(sorted_points):
            ...
            # FIX: Use max_value for scaling
            offset = local_z * sign * (abs(val) / max_value) * diagram_width * beam_length
        ...
        # Create max value annotation
        max_sep = coin.SoSeparator()

        # Create transform for text
        max_transform = coin.SoTransform()
        max_transform.translation.setValue(max_value_point.x, max_value_point.y, max_value_point.z)

        # Rotate text to face camera (simplified version)
        max_transform.rotation.setValue(coin.SbRotation(coin.SbVec3f(0, 1, 0), coin.SbVec3f(0, 0, 1)))

        # Create text
        max_text = coin.SoText2()
        max_text.string.setValue(f"{max_val:.2f}")
        max_text.justification = coin.SoText2.CENTER

        # Text style
        text_font = coin.SoFont()
        text_font.size = 12.0  # Adjust as needed
    ...
    def onListBeams(self, beam_group):
        ...
        for obj in beam_group.Group:
            if hasattr(obj, "Type") and obj.Type in ["BeamFeature", "ResultBeam"]:
                ...
                # Since Length is already a float/number type, we just format it directly.
                # If it were App::PropertyLength, we'd use Units.Quantity(obj.Length).getValueAs('m').Value
                try:
