...
class Solver():
    ...
    def _update_beam_vis(self, obj, members_data):
        ...
        max_val = obj.Results.get_max_diagram_value(obj.LoadCase, key)
        grp = App.ActiveDocument.getObject("BeamsResult")
        if not grp: return
        for b in grp.Group:
            if b.BaseBeam.Name in members_data and key in members_data[b.BaseBeam.Name]:
                ...
                if len(d['values']) >= 2:
                    b.Proxy.set_diagram((value/d['values'][0][-1] for value in d['values'][0]),
                                        d['values'][1], max_val * obj.DiagramScale)
